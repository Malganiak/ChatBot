...
![Overview](images/llama_app_overview.png)
...
- The full code of the application, which can be found on this repository, which I invite you to clone.
- An **access to Llama 2 Models**. To obtain Llama 2, you will need to:
    ...
    - Visit the page of one of the LLaMA 2 available models (version [7B](https://huggingface.co/meta-llama/Llama-2-7b-chat-hf), [13B](https://huggingface.co/meta-llama/Llama-2-13b-chat-hf) or [70B](https://huggingface.co/meta-llama/Llama-2-70b-chat-hf)), and accept Hugging Face's license terms and acceptable use policy. 
...
*If you have cloned this repository, you will not need to rewrite the files (`requirements.txt` and `Dockerfile`) since you already have them. In this case, you can go directly to the "Build the Docker image" step, even if it is better to understand the global process.*
